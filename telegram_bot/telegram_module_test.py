...
def __init__():
    ...
    # update 하고 난 후 최근 아재리스트
    latest_aje_list = doc[0]['latest_aje_list']

    # 추가된 아재 리스트 (aje_list 에 추가될 것)
    new_aje_list =[]
...
def make_DB(new_aje_list):
    ...
    #new_aje_list = ['218404701', '206724413']
    global client
    db = client.telegram_bot_test
    ...
    for k in range(len(new_aje_list)):
        ...
        # webData_form_ajeId 함수로 부터 딕션어리를 리턴받는다.
        # 받는 내용은 위 주석처리 내용과 같다.
        ...
        for i in range(length):
            ...
            data = {
                ...
                'writer': webData_from_ajeId['result_writer'][0],
        ...
        for i,j in enumerate(webData['result_list_content']):
            ...
            if j == doc_content:
                ...
                while(i >= 0):
                    ...
                    bot.sendMessage(chat_id=chat_id, text=webData['result_list_content'][i]+"\n=========================")
                    time.sleep(5)
                    # 디비 저장하고
                    data = {
                        # 'date': result_list_date[len(result_list_date)-i-1],
