...
class kimST_bot:
    ...
    def __init__(self):
        ...
        self.client = MongoClient(TELEGRAM_MONGODB_URL)

        self.db = self.client.telegram_bot_test
    ...
    def aje_list_update_check(self):
        ...
        self.make_new_aje_DB()
        self.update_new_aje_list()

    def get_aje_list(self):
    ...
    def get_new_aje_list(self):
        ##   2020-10-10
        ##   나중엔 중복체크하는 것도 넣어야한다.
        ##   아재 리스트에 있는 아재도 다시 넣을 수 있기 떄문

        aje_list = self.get_aje_list()
        latest_aje_list = self.get_latest_aje_list()
        new_aje_list = []
    ...
    def update_new_aje_list(self):
        new_aje_list = self.get_new_aje_list()

        for k in range(len(new_aje_list)):
            self.db.info.update_one({}, {'$push': {'latest_aje_list': new_aje_list[k]}})


    def get_webData_from_ajeId(self, aje_id):
    ...
    def get_firstDBData_from_ajeId(self, aje_id):
        collection = self.db[aje_id]
    ...
    def make_new_aje_DB(self):
        new_aje_list = self.get_new_aje_list()

        for k in range(len(new_aje_list)):

            # get_webData_from_ajeId 함수로 부터 딕션어리를 리턴받는다.
            webData_from_ajeId = self.get_webData_from_ajeId(new_aje_list[k])
            ...
            for i in range(length):
                ...
                collection = self.db[new_aje_list[k]]
            ...
            self.bot.sendMessage(chat_id=self.chat_id, text="새롭게 추가된 아재 id : ")
            self.bot.sendMessage(chat_id=self.chat_id, text=new_aje_list)


    def content_update_check(self):
        global bot
        global chat_id
        ...
        for k in range(len(aje_list)):
            # 마지막 데이터 가져오기
            firstDBData = self.get_firstDBData_from_ajeId(aje_list[k])
            doc_content = firstDBData['content']
            doc_writer = firstDBData['writer']
            #print(doc_writer)
            webData = self.get_webData_from_ajeId(aje_list[k])
            ...
            for i, j in enumerate(webData['result_list_content']):
                ...
                if j == doc_content:
                    ...
                    while (i >= 0):
                        ...
                        self.bot.sendMessage(chat_id=self.chat_id, text="종목 명 : " + webData['result_list_stockName'][i])
                        self.bot.sendMessage(chat_id=self.chat_id, text=webData['result_list_href'][i])
                        self.bot.sendMessage(chat_id=self.chat_id,
